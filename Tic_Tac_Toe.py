...
def build_Board(board_info, available):
    ...
    print(board_info[0] + ' | ' + board_info[1] + ' | ' + board_info[2] +
          '          ' + available[1] + ' | ' + available[2] + ' | ' + available[3])

    print('---------          ---------')

    print(board_info[3] + ' | ' + board_info[4] + ' | ' + board_info[5] +
          '          ' + available[4] + ' | ' + available[5] + ' | ' + available[6])

    print('---------          ---------')

    print(board_info[6] + ' | ' + board_info[7] + ' | ' + board_info[8] +
          '          ' + available[7] + ' | ' + available[8] + ' | ' + available[9])

def win_Check(board_info, mark):
    top = (board_info[0] == board_info[1] == board_info[2] == mark)
    mid = (board_info[3] == board_info[4] == board_info[5] == mark)
    bot = (board_info[6] == board_info[7] == board_info[8] == mark)
    v1 = (board_info[0] == board_info[3] == board_info[6] == mark)
    v2 = (board_info[1] == board_info[4] == board_info[7] == mark)
    v3 = (board_info[2] == board_info[5] == board_info[8] == mark)
    d1 = (board_info[0] == board_info[4] == board_info[8] == mark)
    d2 = (board_info[2] == board_info[4] == board_info[6] == mark)
        ...
        while current_game:
            ...
            if player_turn == 'Player 1':
                ...
                while win_Check(board_info, p2_marker):
                    ...
                    current_game = False
                    break
                while full_Board(available) == True and win_Check(board_info, p2_marker) == False:
                    print('\nThis game is a draw')
                    build_Board(board_info, available)
                    current_game = False
                    break
